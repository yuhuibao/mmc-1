...
#define SEED_FROM_FILE \
  -999 /**< special flag indicating to load seeds from history file */
#define MIN(a, b) \
  ((a) < (b) ? (a) : (b)) /**< macro to get the min values of two numbers */
...
/***************************************************************************/ /**
 ...
 properties, i.e, the absorption coeff (mu_a)in 1/mm, the scattering coeff
 (mu_s) in 1/mm, the refractive index (n) and anisotropy (g).
 *******************************************************************************/
 ...
 * \brief Filer header data structure to .mch files to store detected photon
 * data This header has a total of 256 bytes
 */
...
typedef struct MMC_history {
  ...
  unsigned int detected; /**< how many photons are detected (not necessarily all
                            saved) */
  unsigned int
      savedphoton; /**< how many detected photons are saved in this file */
  ...
  int respin; /**< if positive, repeat count so total photon=totalphoton*respin;
                 if negative, total number is processed in respin subset */
...
/***************************************************************************/ /**
 ...
 \brief  This structure defines the problem settings (domain, filenames,
 session)
...
typedef struct MMC_config {
  ...
  int srcnum;      /**<total number of sources, could be larger than 1 only with
  ...
  int replaydet; /**<the detector id for which to replay the detected photons,
                    start from 1 0 for wide-field detection pattern*/
  ...
  char ismomentum; /**<1 to save momentum transfer for detected photons, implies
                      issavedet=1*/
  char issaveexit; /**<1 to save the exit position and vector of a detected
                      photon, implies issavedet=1*/
  ...
  char isspecular; /**<1 calculate the initial specular ref if outside the mesh,
                      0 do not calculate*/
  ...
  char isatomic;   /**<1 use atomic operations for weight accumulation, 0 do not
                      use*/
  char method;     /**<0-Plucker 1-Havel, 2-Badouel, 3-branchless Badouel*/
  char basisorder; /**<0 to use piece-wise-constant basis for fluence, 1,
                      linear*/
  char outputtype; /**<'X' output is flux, 'F' output is fluence, 'E' energy
                      deposit*/
  char outputformat;  /**<'ascii' output is text, 'bin': binary, 'json': regular
                         json, 'ubjson': universal binary json*/
  int mcmethod;       /**<0 use MCX-styled MC (micro-Beer-Lambert law), 1 use
                         MCML-styled MC (Albedo-Weight)*/
  ...
  int isextdet; /**<if 1, there is external wide-field detector (marked by -2 in
                   the mesh)*/
  FILE *flog;   /**<stream handle to print log information*/
  char rootpath[MAX_PATH_LENGTH]; /**<a string to specify the root folder of the
                                     simulation*/
  unsigned int
      debuglevel;  /**<a flag to control the printing of the debug information*/
  int debugphoton; /**<if negative, print debug info for all photons, otherwise,
                      only print for the selected one*/
  float unitinmm;  /**<define the length unit in mm*/
  history his;     /**<header info of the history file*/
  unsigned int
      checkpt[MAX_CHECKPOINT]; /**<a list of photon numbers at which a snapshot
                                  of the weights will be saved*/
  ...
  char seedfile[MAX_PATH_LENGTH]; /**<if the seed is specified as a file (mch),
                                     mcx will replay the photons*/
  ...
  double *exportfield;       /*memory buffer when returning the flux to external
                                programs such as matlab*/
  float *exportdetected; /*memory buffer when returning the partial length info
                            to external programs such as matlab*/
  double energytot, energyabs, energyesc;
  unsigned int detectedcount; /**<total number of detected photons*/
  unsigned int runtime;
  char autopilot; /**<1 optimal setting for dedicated card, 2, for non dedicated
                     card*/
  float normalizer; /**<normalization factor*/
  unsigned int
      nbuffer; /**<2^nbuffer is the number of buffers for accummulation*/
  unsigned int gpuid;
} mcconfig;
...
void mcx_savenii(OutputType *dat, size_t len, char *name, int type32bit,
                 int outputformatid, mcconfig *cfg);
void mcx_error(const int id, const char *msg, const char *file,
               const int linenum);
...
#endif
    int
    mmc_throw_exception(const int id, const char *msg, const char *filename,
                        const int linenum);
