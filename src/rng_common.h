...
#define TWO_PI (M_PI * 2.0)
    ...
    float
    rand_next_scatlen(RandType t[RAND_BUF_LEN]) {
 ...
 void rand_next_aangle_sincos(RandType t[RAND_BUF_LEN], float *si,
  ...
  if (pos >= (MATH_BLOCK << 2)) {
    ...
    for (i = 0; i < MATH_BLOCK; i++)
      for (j = 0; j < 4; j++) ran[i].f[j] = TWO_PI * rand_uniform01(t);
    for (i = 0; i < MATH_BLOCK; i++)
...
//! generate [0,1] random number for the next scattering length
 float rand_next_scatlen_ps(RandType t[RAND_BUF_LEN]) {
  ...
  if (pos >= (MATH_BLOCK << 2)) {
    ...
    int i, j;
    for (i = 0; i < MATH_BLOCK; i++)
      for (j = 0; j < 4; j++) ran[i].f[j] = rand_uniform01(t);
    for (i = 0; i < MATH_BLOCK; i++) {
      logval[i].v = log_ps(ran[i].v);
    }
    pos = 0;
  }
  res = ((logval[0].f[pos] != logval[0].f[pos]) ? LOG_RNG_MAX
                                                : (-logval[0].f[pos]));
  pos++;
  return res;
}
