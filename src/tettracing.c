...
/**
 * \brief Tetrahedron faces, in clock-wise orders, represented using local node
 * indices
 *
 * node-connectivity, i.e. nc[4] points to the 4 facets of a tetrahedron, with
 * each triangular face made of 3 nodes. The numbers [0-4] are the local node
 * indices (starting from 0). The order of the nodes are in clock-wise orders.
 */

const int nc[4][3] = {{3, 0, 1}, {3, 1, 2}, {2, 0, 3}, {1, 0, 2}};

/**
 * \brief Tetrahedron faces, in counter-clock-wise orders, represented using
 * local node indices
 *
...
extern const int out[4][3]; /**< defined in simpmesh.c, node orders for each
                               face, in counter-clock-wise orders*/

/**
 * \brief The local index of the node with an opposite face to the i-th face
 * defined in nc[][]
 *
 * nc[i] <-> node[facemap[i]]
...
/**
 * \brief Inverse mapping between the local index of the node and the
 * corresponding opposite face in nc[]'s order
 *
...
/**
 * \brief Index mapping from the i-th face-neighbors (facenb) to the face
 * defined in nc[][]
 *
...
/**
 * \brief Index mapping from the i-th face defined in nc[][] to the
 * face-neighbor (facenb) face orders
 *
 * nc[ifaceorder[i]] <-> facenb[i]
 * nc[0], made of nodes {3,0,1}, is the face connecting to the 4th neighbor
 * (facenb[3]), nc[1], made of nodes {3,1,2}, is the face connecting to the 1st
 * neighbor (facenb[0]), etc.
 */
...
/**
 * \brief function to linearly interpolate between 3 3D points (p1,p2,p3) using
 * weight (w)
 *
...
inline void interppos(float3 *w, float3 *p1, float3 *p2, float3 *p3,
                      float3 *pout) {
...
inline void getinterp(float w1, float w2, float w3, float3 *p1, float3 *p2,
                      float3 *p3, float3 *pout) {
...
void fixphoton(float3 *p, float3 *nodes, int *ee) {
  ...
  for (i = 0; i < 4; i++) vec_add(&c0, nodes + ee[i] - 1, &c0);
...
float plucker_raytet(ray *r, raytracer *tracer, mcconfig *cfg, visitor *visit) {
  ...
  float w[6] = {0.f}, Rv, ww, currweight, dlen = 0.f, rc,
        mus; /*dlen is the physical distance*/
  unsigned int *wi = (unsigned int *)w;
  float baryout[4] = {0.f, 0.f, 0.f, 0.f}, *baryp0 = &(r->bary0.x);
  float Lp0 = 0.f, ratio;

  if (tracer->mesh == NULL || tracer->d == NULL || r->eid <= 0 ||
      r->eid > tracer->mesh->ne)
    return -1;
...
#ifdef __SSE4_1__
      w[i] = _mm_cvtss_f32(
          _mm_add_ss(_mm_dp_ps(O, M, 0x7F), _mm_dp_ps(T, D, 0x7F)));
#else
      M = _mm_mul_ps(O, M);
      M = _mm_hadd_ps(M, M);
      M = _mm_hadd_ps(M, M);
...
#else
  for (i = 0; i < 6; i++)
    w[i] = pinner(&(r->vec), &pcrx, tracer->d + (eid)*6 + i,
                  tracer->m + (eid)*6 + i);
#endif
  // exit(1);
  ...
  for (i = 0; i < 4; i++) {
    // if(cfg->debuglevel&dlTracing) MMC_FPRINTF(cfg->flog,"testing face
    // [%d]\n",i);

    if (i >= 2) w[fc[i][1]] = -w[fc[i][1]];  // can not go back
    if (F32N(wi[fc[i][0]]) & F32N(wi[fc[i][1]]) & F32P(wi[fc[i][2]])) {
      // f_leave
      // if(cfg->debuglevel&dlTracingExit) MMC_FPRINTF(cfg->flog,"ray exits face
      // %d[%d] of %d\n",i,faceorder[i],eid);
      ...
      // if(cfg->debuglevel&dlTracingExit) MMC_FPRINTF(cfg->flog,"exit point %f
...

      {
        ...
        if (r->nexteid > 0) {
          ...
          _mm_prefetch((char *)&((tracer->m + (nexteid)*6 + 4)->x),
                       _MM_HINT_T0);
          _mm_prefetch((char *)&((tracer->d + (nexteid)*6)->x), _MM_HINT_T0);
          _mm_prefetch((char *)&((tracer->d + (nexteid)*6 + 4)->x),
                       _MM_HINT_T0);
        }
#endif
      }
  ...
  visit->raytet++;
  if (tracer->mesh->type[eid] == 0) visit->raytet0++;
  if (r->pout.x != MMC_UNDEFINED) {
    if ((int)((r->photontimer + r->Lmove * rc - cfg->tstart) * visit->rtstep) >=
        (int)((cfg->tend - cfg->tstart) * visit->rtstep)) { /*exit time window*/
      r->faceid = -2;
      r->pout.x = MMC_UNDEFINED;
    ...
    // if(cfg->debuglevel&dlWeight) MMC_FPRINTF(cfg->flog,"update weight to %f
    // and path end %d \n",r->weight,r->isend);
    ...
    if (!cfg->basisorder) {
      ...
      if (cfg->outputtype == otWL || cfg->outputtype == otWP)
        tshift = MIN(((int)(cfg->replaytime[r->photonid] * visit->rtstep)),
                     cfg->maxgate - 1) *
                 tracer->mesh->ne;
      else
        tshift = MIN(((int)((r->photontimer - cfg->tstart) * visit->rtstep)),
                     cfg->maxgate - 1) *
                 tracer->mesh->ne;
      if (cfg->mcmethod == mmMCX) {
...
#pragma omp atomic
          ...
          int psize =
              (int)cfg->srcparam1.w *
              (int)cfg->srcparam2.w;  // total number of pixels in each pattern
          int pidx;                   // pattern index
          for (pidx = 0; pidx < cfg->srcnum; pidx++) {
            if (cfg->isatomic)
#pragma omp atomic
              tracer->mesh->weight[(eid + tshift) * cfg->srcnum + pidx] +=
                  ww * cfg->srcpattern[pidx * psize + r->posidx];
            else
              tracer->mesh->weight[(eid + tshift) * cfg->srcnum + pidx] +=
                  ww * cfg->srcpattern[pidx * psize + r->posidx];
          }
      ...
      if (cfg->debuglevel & dlBary)
        MMC_FPRINTF(cfg->flog, "Y [%f %f %f %f]\n", baryout[0], baryout[1],
                    baryout[2], baryout[3]);
      ...
      if (Lp0 > EPS) {
        ...
        if (prop->mua > 0.f) {
          ...
          if (cfg->outputtype == otWL || cfg->outputtype == otWP)
            ...
            tshift =
                MIN(((int)((r->photontimer - cfg->tstart) * visit->rtstep)),
                    cfg->maxgate - 1) *
                tracer->mesh->nn;

          if (cfg->debuglevel & dlAccum)
            MMC_FPRINTF(cfg->flog, "A %f %f %f %e %d %e\n",
                        r->p0.x - (r->Lmove * 0.5f) * r->vec.x,
                        r->p0.y - (r->Lmove * 0.5f) * r->vec.y,
                        r->p0.z - (r->Lmove * 0.5f) * r->vec.z, ww, eid + 1,
                        dlen);
          ...
          if (cfg->mcmethod == mmMCX) {
            if (cfg->srctype != stPattern) {
              if (cfg->isatomic)
                for (i = 0; i < 4; i++)
#pragma omp atomic
                  tracer->mesh->weight[ee[i] - 1 + tshift] +=
                      ww * (baryp0[i] + baryout[i]);
              else
                for (i = 0; i < 4; i++)
                  tracer->mesh->weight[ee[i] - 1 + tshift] +=
                      ww * (baryp0[i] + baryout[i]);
            } else {
              int psize = (int)cfg->srcparam1.w *
                          (int)cfg->srcparam2
                              .w;  // total number of pixels in each pattern
              int pidx;            // pattern index
              for (pidx = 0; pidx < cfg->srcnum; pidx++) {
                if (cfg->isatomic)
                  for (i = 0; i < 4; i++)
#pragma omp atomic
                    tracer->mesh
                        ->weight[(ee[i] - 1 + tshift) * cfg->srcnum + pidx] +=
                        ww * cfg->srcpattern[pidx * psize + r->posidx] *
                        (baryp0[i] + baryout[i]);
                else
                  for (i = 0; i < 4; i++)
        ...
        if (r->isend) {
          ...
          if (r->nexteid && faceidx >= 0) {
            int j, k,
                *nextenb = (int *)(tracer->mesh->elem +
                                   (r->nexteid - 1) * tracer->mesh->elemlen);
 ...
 * \param[out] bary: output the Barycentric coordinates of the intersection
 * point \param[in] o: current ray origin \param[out] d: current ray direction
 */

inline int havel_sse4(float3 *vecN, float3 *bary, const __m128 o,
                      const __m128 d) {
  const __m128 n = _mm_load_ps(&vecN->x);
  const __m128 det = _mm_dp_ps(n, d, 0x7f);
  if (_mm_movemask_ps(det) & 1) return 0;
  const __m128 dett = _mm_dp_ps(_mm_mul_ps(int_coef, n), o, 0xff);
  const __m128 oldt = _mm_load_ss(&bary->x);

  if ((_mm_movemask_ps(
           _mm_xor_ps(dett, _mm_sub_ss(_mm_mul_ss(oldt, det), dett))) &
       1) == 0) {
    ...
    if ((_mm_movemask_ps(_mm_xor_ps(detu, _mm_sub_ss(det, detu))) & 1) == 0) {
      ...
      if ((_mm_movemask_ps(
               _mm_xor_ps(detv, _mm_sub_ss(det, _mm_add_ss(detu, detv)))) &
           1) == 0) {
        ...
        //_mm_store_ps(&pout->x, _mm_mul_ps(detp,_mm_shuffle_ps(inv_det,
        // inv_det, 0)));
...
float havel_raytet(ray *r, raytracer *tracer, mcconfig *cfg, visitor *visit) {
  ...
  if (tracer->mesh == NULL || tracer->m == NULL || r->eid <= 0 ||
      r->eid > tracer->mesh->ne)
    return -1;
  ...
  for (i = 0; i < 4; i++)
    if (havel_sse4(tracer->m + eid * 12 + i * 3, &bary, O, T)) {
      ...
      if ((int)((r->photontimer + r->Lmove * rc - cfg->tstart) *
                visit->rtstep) >= (int)((cfg->tend - cfg->tstart) *
                                        visit->rtstep)) { /*exit time window*/
        r->faceid = -2;
        r->pout.x = MMC_UNDEFINED;
        r->Lmove = (cfg->tend - r->photontimer) / (prop->n * R_C0) - 1e-4f;
      }
      ...
      if (cfg->debuglevel & dlBary)
        MMC_FPRINTF(cfg->flog, "Y [%f %f %f %f]\n", barypout[0], barypout[1],
                    barypout[2], barypout[3]);
      ...
      if (cfg->debuglevel & dlAccum)
        MMC_FPRINTF(cfg->flog, "A %f %f %f %e %d %e\n",
                    r->p0.x - (r->Lmove * 0.5f) * r->vec.x,
                    r->p0.y - (r->Lmove * 0.5f) * r->vec.y,
                    r->p0.z - (r->Lmove * 0.5f) * r->vec.z, ww, eid + 1, dlen);

      if (r->isend) /* S is the bary centric for the photon after move */
        S = _mm_add_ps(_mm_mul_ps(T, _mm_set1_ps(dlen)),
                       _mm_mul_ps(O, _mm_set1_ps(1.f - dlen)));
      else
        S = T;
      // if(cfg->debuglevel&dlBary)
      //    MMC_FPRINTF(cfg->flog,"old bary0=[%f %f %f
      ...
      if (nextenb && enb) {
        ...
        // if(cfg->debuglevel&dlBary) MMC_FPRINTF(cfg->flog,"[%d %d %d %d],[%d
        // %d %d %d] - ",
        //       ee[0],ee[1],ee[2],ee[3],nextenb[0],nextenb[1],nextenb[2],nextenb[3]);
        // if(cfg->debuglevel&dlBary) MMC_FPRINTF(cfg->flog,"[%f %f %f %f],[%f
      ...
      // if(cfg->debuglevel&dlBary)
      //    MMC_FPRINTF(cfg->flog,"new bary0=[%f %f %f
      //    %f]\n",r->bary0.x,r->bary0.y,r->bary0.z,r->bary0.w);
      if (cfg->mcmethod == mmMCX) {
        if (!cfg->basisorder) {
          if (cfg->srctype != stPattern) {
...
#pragma omp atomic
            ...
            int psize =
                (int)cfg->srcparam1.w *
                (int)
                    cfg->srcparam2.w;  // total number of pixels in each pattern
            int pidx;                  // pattern index
            for (pidx = 0; pidx < cfg->srcnum; pidx++) {
              if (cfg->isatomic)
#pragma omp atomic
...
#pragma omp atomic
                  tracer->mesh
                      ->weight[(ee[j] - 1 + tshift) * cfg->srcnum + pidx] +=
                      barypout[j] * cfg->srcpattern[pidx * psize + r->posidx];
              else
                for (j = 0; j < 4; j++)
                  tracer->mesh
                      ->weight[(ee[j] - 1 + tshift) * cfg->srcnum + pidx] +=
...
float branchless_badouel_raytet(ray *r, raytracer *tracer, mcconfig *cfg,
                                visitor *visit) {
  float3 bary = {1e10f, 0.f, 0.f, 0.f};
  ...
  if (tracer->mesh == NULL || tracer->n == NULL || r->eid <= 0 ||
      r->eid > tracer->mesh->ne)
    return -1;
  ...
  if (r->faceid >= 0 && bary.x >= 0) {
    ...
    enb = (int *)(tracer->mesh->facenb + eid * tracer->mesh->elemlen);
    r->nexteid =
        enb[r->faceid];  // if I use nexteid-1, the speed got slower, strange!
    if (r->nexteid > 0) {
    ...
    if ((int)((r->photontimer + r->Lmove * rc - cfg->tstart) * visit->rtstep) >
        cfg->maxgate - 1) { /*exit time window*/
      r->faceid = -2;
      r->pout.x = MMC_UNDEFINED;
      r->Lmove = (cfg->tend - r->photontimer) / (prop->n * R_C0) - 1e-4f;
    }
    ...
    if (bary.x >= 0.f) {
      ...
      if (cfg->method == rtBLBadouelGrid) framelen = cfg->crop0.z;
      ww = currweight - r->weight;
      r->photontimer += r->Lmove * rc;
...
#pragma omp atomic
            ...
            S = _mm_add_ps(O,
                           _mm_mul_ps(T, _mm_set1_ps(0.5f))); /*starting point*/
            ...
            for (i = 0; i < seg; i++) {
              ...
              unsigned int newidx =
                  idx.z * cfg->crop0.y + idx.y * cfg->crop0.x + idx.x + tshift;
              r->oldidx = (r->oldidx == 0xFFFFFFFF) ? newidx : r->oldidx;
              if (newidx != r->oldidx) {
                tracer->mesh->weight[r->oldidx] += r->oldweight;
                r->oldidx = newidx;
...
/**
 * Havel, Badouel and Branch-less Badouel-based SSE4 ray-tracer require to
 * compile with SSE4 only. Give an error if not compiled with SSE.
 */
...
void onephoton(size_t id, raytracer *tracer, tetmesh *mesh, mcconfig *cfg,
  ...
  float (*engines[5])(ray * r, raytracer * tracer, mcconfig * cfg,
                      visitor * visit) = {
      plucker_raytet, havel_raytet, badouel_raytet, branchless_badouel_raytet,
      branchless_badouel_raytet};
  float (*tracercore)(ray * r, raytracer * tracer, mcconfig * cfg,
                      visitor * visit);
  ...
  if (cfg->srctype != stPattern) {
    if (cfg->seed == SEED_FROM_FILE &&
        (cfg->outputtype == otWL || cfg->outputtype == otWP))
      kahany = cfg->replayweight[r.photonid] -
               visit->kahanc0[0]; /* when replay mode, accumulate detected
                                     photon weight */
    else
...
#pragma omp critical
    {
      for (pidx = 0; pidx < cfg->srcnum; pidx++) {
        if (cfg->seed == SEED_FROM_FILE &&
            (cfg->outputtype == otWL || cfg->outputtype == otWP))
          kahany = cfg->replayweight[r.photonid] -
                   visit->kahanc0[pidx]; /* when replay mode, accumulate
                                            detected photon weight */
        else
          kahany = r.weight * cfg->srcpattern[pidx * psize + r.posidx] -
                   visit->kahanc0[pidx];
  ...
  while (1) { /*propagate a photon until exit*/
    ...
    if (r.pout.x == MMC_UNDEFINED) {
      ...
      if (fixcount++ < MAX_TRIAL) {
        fixphoton(&r.p0, mesh->node,
                  (int *)(mesh->elem + (r.eid - 1) * mesh->elemlen));
        continue;
      }
      r.eid = ID_UNDEFINED;
      r.faceid = -1;
    ...
    if (cfg->issavedet && r.Lmove > 0.f && mesh->type[r.eid - 1] > 0 &&
        r.faceid >= 0)
      r.partialpath[mesh->prop - 1 + mesh->type[r.eid - 1]] +=
    ...
    while (r.faceid >= 0 && !r.isend) {
      ...
      if (cfg->isreflect &&
          (r.eid <= 0 || mesh->med[mesh->type[r.eid - 1]].n !=
                             mesh->med[mesh->type[oldeid - 1]].n)) {
        if (!(r.eid <= 0 &&
              ((mesh->med[mesh->type[oldeid - 1]].n == cfg->nout &&
                cfg->isreflect != (int)bcMirror) ||
               cfg->isreflect == (int)bcAbsorbExterior)))
      ...
      if (mesh->type[oldeid - 1] == 0 && mesh->type[r.eid - 1]) {
        if (cfg->debuglevel & dlExit)
          MMC_FPRINTF(cfg->flog, "e %f %f %f %f %f %f %f %d\n", r.p0.x, r.p0.y,
                      r.p0.z, r.vec.x, r.vec.y, r.vec.z, r.weight, r.eid);
        if (!cfg->voidtime) r.photontimer = 0.f;
      }
    ...
    if (r.eid <= 0 || r.pout.x == MMC_UNDEFINED) {
      ...
      if (r.eid != ID_UNDEFINED) {
        ...
        if (cfg->issavedet && cfg->issaveexit) { /*when issaveexit is set to 1*/
          memcpy(r.partialpath + (visit->reclen - 2 - 6), &(r.p0.x),
                 sizeof(float) *
                     3); /*columns 7-5 from the right store the exit positions*/
          memcpy(r.partialpath + (visit->reclen - 2 - 3), &(r.vec.x),
                 sizeof(float) *
      ...
      if (cfg->issavedet && r.eid <= 0) {
        int i;
        if (cfg->detnum == 0 && cfg->isextdet &&
            mesh->type[oldeid - 1] == mesh->prop + 1) {
          exitdet = oldeid;
        } else
          for (i = 0; i < cfg->detnum; i++) {
            if ((cfg->detpos[i].x - r.p0.x) * (cfg->detpos[i].x - r.p0.x) +
                    (cfg->detpos[i].y - r.p0.y) * (cfg->detpos[i].y - r.p0.y) +
                    (cfg->detpos[i].z - r.p0.z) * (cfg->detpos[i].z - r.p0.z) <
                cfg->detpos[i].w * cfg->detpos[i].w) {
              exitdet = i + 1;
              break;
            }
          }
    ...
    if (cfg->debuglevel & dlMove)
      MMC_FPRINTF(cfg->flog, "M %f %f %f %d %lu %e\n", r.p0.x, r.p0.y, r.p0.z,
                  r.eid, id, r.slen);
    if (cfg->minenergy > 0.f && r.weight < cfg->minenergy &&
        (cfg->tend - cfg->tstart) * visit->rtstep <= 1.f) { /*Russian Roulette*/
    ...
    mom = 0.f;
    r.slen0 = mc_next_scatter(mesh->med[mesh->type[r.eid - 1]].g, &r.vec, ran,
                              ran0, cfg, &mom);
    r.slen = r.slen0;
    if (cfg->mcmethod != mmMCX) albedoweight(&r, mesh, cfg, visit);
    if (cfg->ismomentum &&
        mesh->type[r.eid - 1] > 0) /*when ismomentum is set to 1*/
      r.partialpath[(mesh->prop << 1) - 1 + mesh->type[r.eid - 1]] +=
          mom; /*the third medianum block stores the momentum transfer*/
    r.partialpath[mesh->type[r.eid - 1] -
                  1]++; /*the first medianum block stores the scattering event
                           counts*/
  }
  if (cfg->issavedet && exitdet > 0) {
    ...
    if (visit->bufpos >= visit->detcount) {
      ...
      visit->partialpath = (float *)realloc(
          visit->partialpath, visit->detcount * visit->reclen * sizeof(float));
      if (cfg->issaveseed)
        visit->photonseed =
            realloc(visit->photonseed,
                    visit->detcount * (sizeof(RandType) * RAND_BUF_LEN));
    }
    visit->partialpath[offset] = exitdet;
    memcpy(visit->partialpath + offset + 1, r.partialpath,
           (visit->reclen - 1) * sizeof(float));
    if (cfg->issaveseed)
      memcpy(
          visit->photonseed + visit->bufpos * (sizeof(RandType) * RAND_BUF_LEN),
          r.photonseed, (sizeof(RandType) * RAND_BUF_LEN));
    visit->bufpos++;
  }
  free(r.partialpath);
  if (r.photonseed) free(r.photonseed);
...
#pragma omp critical
    {
      for (pidx = 0; pidx < cfg->srcnum; pidx++) {
        kahany = r.Eabsorb * cfg->srcpattern[pidx * psize + r.posidx] -
                 visit->kahanc1[pidx];
...
float reflectray(mcconfig *cfg, float3 *c0, raytracer *tracer, int *oldeid,
                 int *eid, int faceid, RandType *ran) {
  ...
  n1 = (*oldeid != *eid) ? tracer->mesh->med[tracer->mesh->type[*oldeid - 1]].n
                         : cfg->nout;
  n2 = (*eid > 0) ? tracer->mesh->med[tracer->mesh->type[*eid - 1]].n
                  : cfg->nout;
  ...
  if (tmp2 > 0.f &&
      !(*eid <= 0 &&
        cfg->isreflect == bcMirror)) {     /*if no total internal reflection*/
    ...
    if (rand_next_reflect(ran) <= Rtotal) { /*do reflection*/
      ...
      // if(cfg->debuglevel&dlReflect) MMC_FPRINTF(cfg->flog,"R %f %f %f %d %d
      ...
      *eid = *oldeid; /*stay with the current element*/
      ...
      // if(cfg->debuglevel&dlReflect) MMC_FPRINTF(cfg->flog,"Z %f %f %f %d %d
      // %f\n",c0->x,c0->y,c0->z,*eid,*oldeid,1.f-Rtotal);
    }
  } else { /*total internal reflection*/
    vec_mult_add(pn, c0, -2.f * Icos, 1.f, c0);
    *eid = *oldeid;
    // if(cfg->debuglevel&dlReflect) MMC_FPRINTF(cfg->flog,"V %f %f %f %d %d
...
void launchphoton(mcconfig *cfg, ray *r, tetmesh *mesh, RandType *ran,
                  RandType *ran0) {
  int canfocus = 1;
  float3 origin = {r->p0.x, r->p0.y, r->p0.z};

  r->slen = rand_next_scatlen(ran);
  if (cfg->srctype == stPencil) {  // pencil beam, use the old workflow, except
                                   // when eid is not given
    if (r->eid > 0) return;
  } else if (cfg->srctype == stPlanar || cfg->srctype == stPattern ||
             cfg->srctype == stFourier) {
    ...
    if (cfg->srctype == stPattern) {
      ...
      r->posidx = MIN((int)(ry * ysize), ysize - 1) * xsize +
                  MIN((int)(rx * xsize), xsize - 1);
      if (cfg->seed == SEED_FROM_FILE &&
          (cfg->outputtype == otWL ||
           cfg->outputtype == otWP)) {  // replay mode currently doesn't support
                                        // multiple source patterns
        r->weight = cfg->srcpattern[MIN((int)(ry * cfg->srcparam2.w),
                                        (int)cfg->srcparam2.w - 1) *
                                        (int)(cfg->srcparam1.w) +
                                    MIN((int)(rx * cfg->srcparam1.w),
                                        (int)cfg->srcparam1.w - 1)];
        cfg->replayweight[r->photonid] *= r->weight;
      }
    } else if (cfg->srctype == stFourier) {
      r->weight =
          (cosf((floorf(cfg->srcparam1.w) * rx + floorf(cfg->srcparam2.w) * ry +
                 cfg->srcparam1.w - floorf(cfg->srcparam1.w)) *
                TWO_PI) *
               (1.f - cfg->srcparam2.w + floorf(cfg->srcparam2.w)) +
           1.f) *
          0.5f;
    }
    ...
    v2.w *= 1.f / (sqrt(cfg->srcparam1.x * cfg->srcparam1.x +
                        cfg->srcparam1.y * cfg->srcparam1.y +
                        cfg->srcparam1.z * cfg->srcparam1.z));
    v2.x = v2.w * (cfg->srcdir.y * cfg->srcparam1.z -
                   cfg->srcdir.z * cfg->srcparam1.y);
    v2.y = v2.w * (cfg->srcdir.z * cfg->srcparam1.x -
                   cfg->srcdir.x * cfg->srcparam1.z);
    v2.z = v2.w * (cfg->srcdir.x * cfg->srcparam1.y -
                   cfg->srcdir.y * cfg->srcparam1.x);
    ...
    if (cfg->srctype == stFourier2D)
      r->weight =
          (sinf((cfg->srcparam2.x * rx + cfg->srcparam2.z) * TWO_PI) *
               sinf((cfg->srcparam2.y * ry + cfg->srcparam2.w) * TWO_PI) +
           1.f) *
          0.5f;  // between 0 and 1
    else
      r->weight = (cosf((cfg->srcparam2.x * rx + cfg->srcparam2.y * ry +
                         cfg->srcparam2.z) *
                        TWO_PI) *
                       (1.f - cfg->srcparam2.w) +
                   1.f) *
    ...
    origin.z += (cfg->srcparam1.z + v2.z) * 0.5f;
             ...
             cfg->srctype == stGaussian) {  // uniform disk and Gaussian beam
    float sphi, cphi;
    ...
    if (cfg->srctype == stDisk)
      ...
      float z0 = cfg->srcparam1.x * cfg->srcparam1.x * M_PI /
                 cfg->srcparam1.y;  // Rayleigh range
      r0 = sqrt(-log(rand_uniform01(ran)) *
                (1.f + (r->focus * r->focus / (z0 * z0)))) *
           cfg->srcparam1.x;
    }
    if (cfg->srcdir.z > -1.f + EPS && cfg->srcdir.z < 1.f - EPS) {
      float tmp0 = 1.f - cfg->srcdir.z * cfg->srcdir.z;
      float tmp1 = r0 / sqrt(tmp0);
      r->p0.x = cfg->srcpos.x + tmp1 * (cfg->srcdir.x * cfg->srcdir.z * cphi -
                                        cfg->srcdir.y * sphi);
      r->p0.y = cfg->srcpos.y + tmp1 * (cfg->srcdir.y * cfg->srcdir.z * cphi +
                                        cfg->srcdir.x * sphi);
  ...
  } else if (cfg->srctype == stCone || cfg->srctype == stIsotropic ||
             cfg->srctype == stArcSin) {
      ...
      do {
        ang =
            (cfg->srcparam1.y > 0)
                ...
                : acosf(2.f * rand_uniform01(ran) - 1.f);  // sine distribution
      } while (ang > cfg->srcparam1.x);
    } else {
      if (cfg->srctype == stIsotropic)                 // uniform sphere
        ang = acosf(2.f * rand_uniform01(ran) - 1.f);  // sine distribution
      else
        ang = M_PI * rand_uniform01(
                         ran);  // uniform distribution in zenith angle, arcsine
    }
    stheta = sinf(ang);
    ctheta = cosf(ang);
    r->vec.x = stheta * cphi;
    ...
    sphi = sinf(ang);
    cphi = cosf(ang);
    ang = sqrt(-2.f * log(rand_uniform01(ran))) *
          (1.f - 2.f * rand_uniform01(ran0)) * cfg->srcparam1.x;
    stheta = sinf(ang);
    ctheta = cosf(ang);
    r->vec.x = stheta * cphi;
    r->vec.y = stheta * sphi;
    ...
    if (cfg->srctype == stLine) {
      ...
      p = sqrt(1.f - r->vec.x * r->vec.x - r->vec.y * r->vec.y) *
  ...
  if (canfocus &&
      r->focus != 0.f) {  // if beam focus is set, determine the incident angle
    float Rn2;
  ...
  for (is = 0; is < mesh->srcelemlen; is++) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      vec_cross(&vecAB, &vecAC,
                &vecN);  // repeated for all photons, vecN can be precomputed
...
#pragma omp critical
    {
      MMC_FPRINTF(
          cfg->flog,
          "all tetrahedra (%d) labeled with -1 do not enclose the source!\n",
          mesh->srcelemlen);
      if (mesh->srcelemlen) {
        int *elems =
            (int *)(mesh->elem + (mesh->srcelem[0] - 1) * mesh->elemlen);
        MMC_FPRINTF(cfg->flog, "elem %d %d [%f %f %f] \n", mesh->srcelem[0],
                    elems[0], mesh->node[elems[0] - 1].x,
                    mesh->node[elems[0] - 1].y, mesh->node[elems[0] - 1].z);
        MMC_FPRINTF(cfg->flog, "elem %d %d [%f %f %f] \n", mesh->srcelem[0],
                    elems[1], mesh->node[elems[1] - 1].x,
                    mesh->node[elems[1] - 1].y, mesh->node[elems[1] - 1].z);
        MMC_FPRINTF(cfg->flog, "elem %d %d [%f %f %f] \n", mesh->srcelem[0],
                    elems[2], mesh->node[elems[2] - 1].x,
                    mesh->node[elems[2] - 1].y, mesh->node[elems[2] - 1].z);
        MMC_FPRINTF(cfg->flog, "elem %d %d [%f %f %f] \n", mesh->srcelem[0],
                    elems[3], mesh->node[elems[3] - 1].x,
                    mesh->node[elems[3] - 1].y, mesh->node[elems[3] - 1].z);
...
void albedoweight(ray *r, tetmesh *mesh, mcconfig *cfg, visitor *visit) {
  ...
  int i, tshift,
      datalen = (cfg->method == rtBLBadouelGrid)
                    ? cfg->crop0.z
