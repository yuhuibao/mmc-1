 ...
 **          in Pl�cker Coordinates,"</a> Biomed. Opt. Express, 1(1) 165-175
 *(2010).
...
typedef struct MMC_mesh {
  ...
  int *
      detelem; /**< candidate list of elements containing a widefield detector*/
...
#endif
    int
    mcx_throw_exception(const int id, const char *msg, const char *filename,
                        const int linenum);
#endif
...
static inline void vec_mult_add(float3 *a, float3 *b, float sa, float sb,
                                float3 *res) {
  res->x = sb * b->x + sa * a->x;
  res->y = sb * b->y + sa * a->y;
  res->z = sb * b->z + sa * a->z;
...
static inline float dist(float3 *p0, float3 *p1) { return sqrt(dist2(p0, p1)); }
