 ...
 * indices (starting from 0). The order of the nodes are in counter-clock-wise
 * orders.
 */
...
void mesh_init_from_cfg(tetmesh *mesh, mcconfig *cfg) {
  ...
  if (cfg->basisorder == 2) mesh_10nodetet(mesh, cfg);
...
void mesh_loadnode(tetmesh *mesh, mcconfig *cfg) {
  ...
  for (i = 0; i < mesh->nn; i++) {
    if (fscanf(fp, "%d %f %f %f", &tmp, &(mesh->node[i].x), &(mesh->node[i].y),
               &(mesh->node[i].z)) != 4)
      MESH_ERROR("node file has wrong format");
  }
  fclose(fp);
  if (cfg->method == rtBLBadouelGrid) mesh_createdualmesh(mesh, cfg);
}
...
void mesh_loadmedia(tetmesh *mesh, mcconfig *cfg) {
  ...
  if (cfg->isextdet) {
    ...
    for (i = 0; i < mesh->ne; i++) {
      if (mesh->type[i] == -2) mesh->type[i] = mesh->prop + 1;
    }
  }
  for (i = 1; i <= mesh->prop; i++) {
    if (fscanf(fp, "%d %f %f %f %f", &tmp, &(mesh->med[i].mua),
               &(mesh->med[i].mus), &(mesh->med[i].g), &(mesh->med[i].n)) != 5)
      MESH_ERROR("property file has wrong format");
...
void mesh_loadelem(tetmesh *mesh, mcconfig *cfg) {
  ...
  if (mesh->elemlen < 4) mesh->elemlen = 4;
  ...
  mesh->weight =
      (double *)calloc(sizeof(double) * datalen, cfg->maxgate * cfg->srcnum);

  for (i = 0; i < mesh->ne; i++) {
    pe = mesh->elem + i * mesh->elemlen;
    if (fscanf(fp, "%d", &tmp) != 1) break;
    for (j = 0; j < mesh->elemlen; j++)
      if (fscanf(fp, "%d", pe + j) != 1) break;
    if (fscanf(fp, "%d", mesh->type + i) != 1) break;
  }
  fclose(fp);
  if (i < mesh->ne) MESH_ERROR("element file has wrong format");
  mesh_srcdetelem(mesh, cfg);
}

/**
 * @brief Identify wide-field source and detector-related elements (type=-1 for
 * source, type=-2 for det)
 *
 * @param[in] mesh: the mesh object
 * @param[in] cfg: the simulation configuration structure
 */
...
void mesh_srcdetelem(tetmesh *mesh, mcconfig *cfg) {
  ...
  for (i = 0; i < mesh->ne; i++) {
    if (mesh->type[i] ==
        -1) { /*number of elements in the initial candidate list*/
      mesh->srcelemlen++;
      cfg->e0 = (cfg->e0 == 0) ? i + 1 : cfg->e0;
    }
    if (mesh->type[i] ==
        -2) { /*number of elements in the initial candidate list*/
      mesh->detelemlen++;
      cfg->isextdet = 1;
      cfg->detnum =
          0;  // when detecting wide-field detectors, suppress point detectors
    }
  }
      ...
      if (mesh->type[i] < 0) {
        if (mesh->type[i] == -1) {
          ...
          mesh->type[i] = 0;
                   ...
                   -2) /*keep -2, will be replaced to medianum+1 in loadmedia*/
          mesh->detelem[id++] = i + 1;
      }
    }
  }
...
/**
 * @brief Load tet element volume file and initialize the related mesh
 * properties
 *
 * @param[in] mesh: the mesh object
 * @param[in] cfg: the simulation configuration structure
...
/**
 * @brief Load face-neightbor element list and initialize the related mesh
 * properties
 *
 * @param[in] mesh: the mesh object
 * @param[in] cfg: the simulation configuration structure
...
void mesh_loadseedfile(tetmesh *mesh, mcconfig *cfg) {
  ...
  if (fp == NULL) MESH_ERROR("can not open the specified history file");
  ...
  if (cfg->photonseed == NULL) MESH_ERROR("can not allocate memory");
  if (fread(cfg->photonseed, his.seedbyte, his.savedphoton, fp) !=
      his.savedphoton)
    MESH_ERROR("error when reading the seed data");
  cfg->seed = SEED_FROM_FILE;
  cfg->nphoton = his.savedphoton;

  if (cfg->outputtype == otJacobian || cfg->outputtype == otWL ||
      cfg->outputtype == otWP || cfg->replaydet > 0) {
    int i, j;
    float *ppath =
        (float *)malloc(his.savedphoton * his.colcount * sizeof(float));
    ...
    if (fread(ppath, his.colcount * sizeof(float), his.savedphoton, fp) !=
        his.savedphoton)
    ...
    for (i = 0; i < his.savedphoton; i++)
      if (cfg->replaydet == 0 ||
          cfg->replaydet == (int)(ppath[i * his.colcount])) {
        memcpy((char *)(cfg->photonseed) + cfg->nphoton * his.seedbyte,
               (char *)(cfg->photonseed) + i * his.seedbyte, his.seedbyte);
        // replay with wide-field detection pattern, the partial path has to
        // contain photon exit information
        if ((cfg->detparam1.w * cfg->detparam2.w > 0) &&
            (cfg->detpattern != NULL)) {
          cfg->replayweight[cfg->nphoton] =
              mesh_getdetweight(i, his.colcount, ppath, cfg);
        } else {
          cfg->replayweight[cfg->nphoton] = ppath[(i + 1) * his.colcount - 1];
        }
        for (j = 2; j < his.maxmedia + 2; j++)
          cfg->replayweight[cfg->nphoton] *=
              expf(-mesh->med[j - 1].mua * ppath[i * his.colcount + j] *
                   his.unitinmm);
        cfg->replaytime[cfg->nphoton] = 0.f;
        for (j = 2; j < his.maxmedia + 2; j++)
          cfg->replaytime[cfg->nphoton] +=
              mesh->med[j - 1].n * ppath[i * his.colcount + j] * R_C0;
        cfg->nphoton++;
      }
    free(ppath);
    cfg->photonseed = realloc(cfg->photonseed, cfg->nphoton * his.seedbyte);
    cfg->replayweight =
        (float *)realloc(cfg->replayweight, cfg->nphoton * sizeof(float));
    cfg->replaytime =
        (float *)realloc(cfg->replaytime, cfg->nphoton * sizeof(float));
    cfg->minenergy = 0.f;
  }
  fclose(fp);
}
 ...
 *
 * Destructor of the mesh data structure, delete all dynamically allocated
 * members
 */
...
/**
 * @brief Initialize a data structure storing all pre-computed ray-tracing
 * related data
 *
...
void tracer_prep(raytracer *tracer, mcconfig *cfg) {
  ...
  } else if ((cfg->srctype == stPencil || cfg->srctype == stIsotropic ||
              cfg->srctype == stCone || cfg->srctype == stArcSin) &&
             cfg->e0 > 0) {
    int eid = cfg->e0 - 1;
    float3 vecS = {0.f}, *nodes = tracer->mesh->node, vecAB, vecAC, vecN;
    int ea, eb, ec;
    float s = 0.f, *bary = &(cfg->bary0.x);
    int *elems = (int *)(tracer->mesh->elem +
                         eid * tracer->mesh->elemlen);  // convert int4* to int*
  ...
  if (tracer->mesh->dref) free(tracer->mesh->dref);
  if (cfg->issaveref)
    tracer->mesh->dref = (double *)calloc(
        sizeof(double) * tracer->mesh->nf * cfg->srcnum, cfg->maxgate);
}
 ...
 * m: the cross-product of the end-nodes of the 6-edges: edge[i]=[ni1,ni2]:
 * m[i]=ni1 x ni2 n: the normal vector of the 4 facets, pointing outwards
 *
 * @param[out] tracer: the ray-tracer data structure
 */
  ...
  if (tracer->method == rtPlucker) {
    ...
    for (i = 0; i < ne; i++) {
      ...
      for (j = 0; j < 4; j++) {
        ...
        vec_cross(
            &vecAB, &vecAC,
            vecN); /*N is defined as ACxAB in Jiri's code, but not the paper*/
    ...
    for (i = 0; i < ne; i++) {
      ...
      for (j = 0; j < 4; j++) {
        ...
        vec_cross(
            &vecAB, &vecAC,
            &vN); /*N is defined as ACxAB in Jiri's code, but not the paper*/
...
float mc_next_scatter(float g, float3 *dir, RandType *ran, RandType *ran0,
                      mcconfig *cfg, float *pmom) {
  float nextslen;
  ...
  // Henyey-Greenstein Phase Function, "Handbook of Optical Biomedical
  // Diagnostics",2002,Chap3,p234 see Boas2002
  ...
  if (cfg->ismomentum) pmom[0] += (1.f - ctheta);
...
void mesh_saveweight(tetmesh *mesh, mcconfig *cfg, int isref) {
  ...
  if (cfg->rootpath[0])
    sprintf(fweight, "%s%c%s%s.dat", cfg->rootpath, pathsep, cfg->session,
            (isref ? "_dref" : ""));
  else
 ...
 *
 * @param[in] ppath: buffer points to the detected photon data (partial-path,
 * det id, etc)
...
void mesh_savedetphoton(float *ppath, void *seeds, int count, int seedbyte,
                        mcconfig *cfg) {
  FILE *fp;
  ...
  if (cfg->method != rtBLBadouelGrid) cfg->his.unitinmm = cfg->unitinmm;
  ...
  cfg->his.colcount = (2 + (cfg->ismomentum > 0)) * cfg->his.maxmedia +
                      (cfg->issaveexit > 0) * 6 + 2; /*column count=maxmedia+3*/

  if (count > 0 && cfg->exportdetected == NULL) {
    cfg->detectedcount = count;
    cfg->exportdetected =
        (float *)malloc(cfg->his.colcount * cfg->detectedcount * sizeof(float));
  }
  ...
  if (cfg->issaveseed && seeds != NULL) fwrite(seeds, seedbyte, count, fp);
  fclose(fp);
}

/**
 * @brief Save binned detected photon data over an area-detector as
 * time-resolved 2D images
 *
 * When an area detector is used (such as a CCD), storing all detected photons
 * can generate a huge output file. This can be mitigated by accumulate the data
 * first to a rasterized detector grid, and then save only the integrated data.
 *
...
void mesh_getdetimage(float *detmap, float *ppath, int count, mcconfig *cfg,
                      tetmesh *mesh) {
  ...
  for (i = 0; i < count; i++) {
    ...
    for (j = 1; j <= cfg->his.maxmedia; j++) {
      ...
      weight *= expf(-ppath[i * colcount + j + cfg->his.maxmedia] *
                     mesh->med[j].mua * unitinmm);
    }
    ntg = (int)path * R_C0 / cfg->tstep;
    if (ntg > cfg->maxgate - 1) ntg = cfg->maxgate - 1;
...
void mesh_savedetimage(float *detmap, mcconfig *cfg) {
  ...
  fwrite(detmap, sizeof(float),
         cfg->detparam1.w * cfg->detparam2.w * cfg->maxgate, fp);
  fclose(fp);
}
...
float mesh_getdetweight(int photonid, int colcount, float *ppath,
                        mcconfig *cfg) {
  float x0 = cfg->detpos[0].x;
  float y0 = cfg->detpos[0].y;
  float xrange = cfg->detparam1.x + cfg->detparam2.x;
...
/**
 * @brief Function to normalize the fluence and remove influence from photon
 * number and volume
 *
 * This function outputs the Green's function from the raw simulation data. This
 * needs to divide the total simulated photon energy, normalize the volumes of
 * each node/elem, and consider the length unit and time-gates
 *
 * @param[in] mesh: the mesh object
 * @param[in] cfg: the simulation configuration
 * @param[in] Eabsorb: total absorbed energy from ray-tracing accummulation
 * @param[in] Etotal: total launched energy, equal to photon number if not
 * pattern-source
 */

/*see Eq (1) in Fang&Boas, Opt. Express, vol 17, No.22, pp. 20178-20190, Oct
 * 2009*/
float mesh_normalize(tetmesh *mesh, mcconfig *cfg, float Eabsorb, float Etotal,
  ...
  int *ee;
  int datalen = (cfg->method == rtBLBadouelGrid)
                    ? cfg->crop0.z
                    : ((cfg->basisorder) ? mesh->nn : mesh->ne);
  ...
  if (cfg->issaveref && mesh->dref) {
    ...
    for (i = 0; i < cfg->maxgate; i++)
      for (j = 0; j < mesh->nf; j++) mesh->dref[i * mesh->nf + j] *= normalizor;
  }

  if (cfg->seed == SEED_FROM_FILE &&
      (cfg->outputtype == otJacobian || cfg->outputtype == otWL ||
       cfg->outputtype == otWP)) {
    float normalizor = 1.f / (DELTA_MUA * cfg->nphoton);
    if (cfg->outputtype == otWL || cfg->outputtype == otWP)
      normalizor =
          1.f /
          Etotal; /*Etotal is total detected photon weight in the replay mode*/
  ...
  if (cfg->method == rtBLBadouelGrid) {
    normalizor = 1.0 / (Etotal * cfg->unitinmm * cfg->unitinmm *
                        cfg->unitinmm); /*scaling factor*/
  } else {
    if (cfg->basisorder) {
      for (i = 0; i < cfg->maxgate; i++)
        for (j = 0; j < datalen; j++)
          if (mesh->nvol[j] > 0.f)
            mesh->weight[(i * datalen + j) * cfg->srcnum + pair] /=
                mesh->nvol[j];
      ...
      for (i = 0; i < mesh->ne; i++) {
        ...
        for (j = 0; j < cfg->maxgate; j++)
          for (k = 0; k < 4; k++)
            energyelem +=
                mesh->weight[(j * mesh->nn + ee[k] - 1) * cfg->srcnum +
                             pair]; /*1/4 factor is absorbed two lines below*/
        energydeposit +=
            energyelem * mesh->evol[i] *
            mesh->med[mesh->type[i]].mua; /**mesh->med[mesh->type[i]].n;*/
      }
      normalizor =
          Eabsorb / (Etotal * energydeposit * 0.25f); /*scaling factor*/
    } else {
      for (i = 0; i < datalen; i++)
        for (j = 0; j < cfg->maxgate; j++)
  ...
  if (cfg->outputtype == otFlux) normalizor /= cfg->tstep;
